...
CSameGameBoard::CSameGameBoard()
	: m_arrBoard(NULL), m_nColumns(15), m_nRows(15),
	m_nHeight(35), m_nWidth(35), m_nRemaining(0), m_nColors(3)
{
	m_arrColors[0] = RGB(0, 0, 0);		// белый фон
	m_arrColors[1] = RGB(255, 0, 0);	// красный
	m_arrColors[2] = RGB(255, 255, 64);	// желтый
	m_arrColors[3] = RGB(0, 0, 255);	// синий
	m_arrColors[4] = RGB(0, 255, 0);	// зеленый
	m_arrColors[5] = RGB(0, 255, 255);	// голубой
	m_arrColors[6] = RGB(255, 0, 128);	// розовый
	m_arrColors[7] = RGB(0, 64, 0);		// темно-зеленый
	...
	Direction direction) 
{
	...
	if (direction != DIRECTION_UP)
		nCount += DeleteNeighborBlocks(row + 1, col, color, DIRECTION_DOWN);
	// Если мы пришли НЕ СЛЕВА, то идем НАЛЕВО
	if (direction != DIRECTION_UP)
		nCount += DeleteNeighborBlocks(row, col - 1, color, DIRECTION_DOWN);
	// Если мы пришли НЕ СПРАВА, то идем НАПРАВО
	if (direction != DIRECTION_UP)
		nCount += DeleteNeighborBlocks(row, col + 1, color, DIRECTION_DOWN);
